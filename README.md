...
- **`dv:vps-health`** — snapshot `openclaw-prod`'s runtime health in one SSH call: Hermes gateway and cron scheduler, 24h cron failures (including the silent delivery errors that swallow output), Axiom tmux, host memory and load, feed freshness, SSH brute-force pressure, and cold-backup integrity — then interpret each section. Host-specific by design; read-only.
