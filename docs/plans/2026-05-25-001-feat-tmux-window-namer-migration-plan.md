...
- **`allowed-tools` — dropped during PR review (was: keep top-level).** Plan-time decision was to keep a top-level `allowed-tools` key (validator-verified to pass skills-ref 0.1.1). **Reversed in PR #11 review:** the inherited allowlist omitted commands the core apply/persist path needs (`python3` for PUA-safe glyph writes, `bash` for the persistence script), and `Bash(cmd *)` prefix-matching can't reliably cover the skill's compound commands and heredocs. Rather than maintain a fragile, incomplete allowlist, the skill declares no `allowed-tools` and runs under normal permission flow, matching `pickup-handoff`'s skills.
...
- **(b) `allowed-tools`:** Recognized top-level spec key (Experimental); skills-ref 0.1.1 accepts it. **Dropped during PR #11 review** because the inherited list omitted core-path commands (`python3`, `bash`) and can't cover compound commands/heredocs — see Key Technical Decisions.
