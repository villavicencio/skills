...
- **`allowed-tools` retained comma-separated.** The agentskills spec describes a space-separated value, but these are parameterized patterns (`Bash(tmux *)`) with internal spaces; comma-separation is the form Claude Code parses correctly, and the `skills-ref` 0.1.1 validator accepts it.
